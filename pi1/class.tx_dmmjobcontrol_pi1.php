...
class tx_dmmjobcontrol_pi1 extends tslib_pibase {
			...
			foreach ($displayCodesArray AS $displayCode) {
				...
				switch ($displayCode) {
					...
					case 'RSS':
						...
						$content .= '<?xml version="1.0" encoding="iso-8859-1"?>'.$this->displayList($this->conf['template.']['rss']);
					break;
	...
	function getFormSelect($field) {
		...
		if (isset($TCA['tx_dmmjobcontrol_job']['columns'][$field]['config']['MM'])) {
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				$selected = '';
				if ($session['search'][$field] == $row['uid']) {
					$selected = ' selected';
				}
		...
		if ($this->conf['show_icon'] && $selected == ' selected') {
